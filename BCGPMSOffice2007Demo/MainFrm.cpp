...
BEGIN_MESSAGE_MAP(CMainFrame, CBCGPFrameWnd)
	...
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_2007_1, ID_VIEW_APPLOOK_2007_5, OnUpdateAppLook)
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_Pat[1].Create (CBCGPControlRendererParams (IDB_VIEWPAT_2, CLR_DEFAULT, 
		CRect (0, 0, 499, 157), CRect (0, 0, 0, 0), CRect (480, 0, 0, 0), CRect (480, 0, 499, 157)));
	m_Pat[2].Create (CBCGPControlRendererParams (IDB_VIEWPAT_3, CLR_DEFAULT, 
		CRect (0, 0, 499, 157), CRect (0, 0, 0, 0), CRect (480, 0, 0, 0), CRect (480, 0, 499, 157)));
	m_Pat[3].Create (CBCGPControlRendererParams (IDB_VIEWPAT_4, CLR_DEFAULT, 
		CRect (0, 0, 499, 157), CRect (0, 0, 0, 0), CRect (480, 0, 0, 0), CRect (480, 0, 499, 157)));	
	...
	if (!m_wndClipboardPane.Create(_T("Clipboard"), this, TRUE, IDD_CLIPBOARD, 
		WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_BCGP_CLOSE | CBRS_BCGP_FLOAT,
		ID_VIEW_TASK_PANE))
	{
	...
	m_wndClipboardPane.EnableDocking (CBRS_ALIGN_RIGHT | CBRS_ALIGN_LEFT);
...
void CMainFrame::OnViewApplookCustomColor() 
{
	...
	OnAppLook(ID_VIEW_APPLOOK_2007_5);
}
...
BOOL CMainFrame::CreateRibbonBar ()
{
	...
	//----------------------------------------
	CBCGPRibbonButton* pVisualStyleButton = new CBCGPRibbonButton (ID_SET_STYLE, _T("Style"), -1, -1);

	pVisualStyleButton->SetMenu (IDR_THEME_MENU, FALSE /* No default command */, TRUE /* Right align */);

	int nIndex = pVisualStyleButton->FindSubItemIndexByID (ID_VIEW_APPLOOK_2007_5);
	if (nIndex != -1)
	{
		pVisualStyleButton->RemoveSubItem (nIndex);

		CreateLookColor ();
		pVisualStyleButton->AddSubItem (m_pLookColor, nIndex);
	}	
...
void CMainFrame::AddTab_Home ()
{
	...
	pPanelClipboard->Add (new CBCGPRibbonButton (ID_EDIT_COPYFORMAT, _T("Format Painter"), 3));
	...
	pFontCombo->SetWidth (60, TRUE);	// Width in "floaty" mode
	pFontCombo->SelectItem (10);
	pFontGroup->AddButton (pFontCombo);

	CBCGPRibbonComboBox* pFontSizeCombo = new CBCGPRibbonComboBox (ID_FONT_FONTSIZE, TRUE, 39);
	...
	pFontSizeCombo->SetWidth (24, TRUE);	// Width in "floaty" mode
	...
	pFontColorHighlightBtn->SetColor ((COLORREF)-1);
	...
	pFontColorHighlightBtn->AddSubItem (new CBCGPRibbonButton (ID_STOP_HIGHLIGHTING, _T("&Stop Highlighting")));
	...
	//--------------------------------------
	CBCGPRibbonPaletteButton* pPaletteMulti = new CBCGPRibbonPaletteButton (ID_PARA_MULTILEVEL, _T(""), -1, -1);
	...
	pParaColorBtn->SetColor ((COLORREF)-1);
	...
	CBCGPRibbonPaletteButton* pStyleBtn = new CBCGPRibbonPaletteButton 
		(ID_WRITE_QUICKSTYLES, _T("Quick Styles"), 9, 1, IDB_STYLES, 64);
	...
	CBCGPRibbonButton* pStyleChangeBtn = new CBCGPRibbonButton (ID_WRITE_CHANGESTYLES, _T("Change Styles"), 9, 2);
	...
	pStyleSetPalette->AddSubItem (new CBCGPRibbonButton (0, _T("&Reset to Quick Styles from Template")));
	pStyleSetPalette->EnableMenuResize (TRUE, TRUE);	// Vertical only
	pStyleSetPalette->EnablMenuSideBar ();
	...
	pColorsPalette->AddSubItem (new CBCGPRibbonButton (ID_PAGELAYOUT_COLORS_CREATENEWTHEMECOLORS, _T("&Create New Theme Colors...")));
	...
	pFontsPalette->AddSubItem (new CBCGPRibbonButton (ID_PAGELAYOUT_FONTS_CREATENEWTHEMEFONTS, _T("&Create New Theme Fonts...")));
	...
	//---------------------
	CBCGPRibbonPanel* pPanelEdit = pCategory->AddPanel (
		_T("Editing"), 
		m_PanelImages.ExtractIcon (5));
	pPanelEdit->SetKeys (_T("zn"));

	CBCGPRibbonButton* pFindBtn = new CBCGPRibbonButton (ID_EDIT_FIND, _T("Find"), 4);
	pFindBtn->SetMenu (IDR_WRITE_FIND_MENU);
	pFindBtn->SetDefaultCommand (ID_EDIT_FIND);
	pPanelEdit->Add (pFindBtn);

	pPanelEdit->Add (new CBCGPRibbonButton (ID_EDIT_REPLACE, _T("Replace"), 5));
	...
	//--------------------------
	pCategory->AddHidden (new CBCGPRibbonUndoButton (ID_EDIT_UNDO, _T("Undo"), 10));

	pCategory->AddHidden (new CBCGPRibbonUndoButton (ID_EDIT_FIND, _T("Find"), 4));
	pCategory->AddHidden (new CBCGPRibbonUndoButton (ID_EDIT_GOTO, _T("Go To"), 6));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_EDIT_PASTE_SPECIAL, _T("Paste Special"), 11));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_EDIT_PASTE_HYPERLINK, _T("Paste as Hyperlink"), 12));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_PARA_CHANGE_LIST_LEVEL, _T("Change List Level"), 13));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_PARA_LINESPACINGBEFORE, _T("Add Space Before Paragraph"), 14));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_PARA_LINESPACINGAFTER, _T("Remove Space After Paragraph"), 15));

    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_BOTTOMBORDER, _T("&Bottom Border"), 16));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_TOPBORDER, _T("To&p Border"), 17));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_LEFTBORDER, _T("&Left Border"), 18));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_RIGHTBORDER, _T("&Right Border"), 19));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_NOBORDER, _T("&No Border"), 20));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_ALLBORDERS, _T("&All Borders"), 21));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_OUTSIDEBORDERS, _T("Out&side Borders"), 22));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_INSIDEBORDERS, _T("&Inside Borders"), 23));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_INSIDEHORIZONTALBORDER, _T("Inside &Horizontal Border"), 24));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_INSIDEVERTICALBORDER, _T("Inside &Vertical Border"), 25));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_DIAGONALDOWNBORDER, _T("Diagonal &Down Border"), 26));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_DIAGONALUPBORDER, _T("Diagonal &Up Border"), 27));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_HORIZONTALLINE, _T("Hori&zontal Line"), 28));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_DRAWTABLE, _T("&Draw Table"), 29));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_VIEWGRIDLINES, _T("View &Gridlines"), 30));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_BORDERS_BORDERSANDSHADING, _T("B&orders and Shading..."), 31));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_EDIT_SELECT_ALL, _T("Select All"), 35));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_EDIT_SELECTOBJECTS, _T("Select Objects"), 7));
...
void CMainFrame::AddTab_Insert ()
{
	...
	CBCGPRibbonPanel* pPanelPages = pCategory->AddPanel (
		_T("Pages"), m_PanelImages.ExtractIcon (6));

	pPanelPages->SetKeys (_T("zp"));

	CBCGPRibbonButton* pInsertCoverPageBtn = new CBCGPRibbonButton (ID_INSERT_COVERPAGE, _T("Cover Page"), 0, 0);
	pInsertCoverPageBtn->SetMenu (IDR_INSERT_COVERPAGE_MENU);
	pPanelPages->Add (pInsertCoverPageBtn);

	pPanelPages->Add (new CBCGPRibbonButton (ID_INSERT_BLACKPAGE, _T("Blank Page"), 1, 1));
	pPanelPages->Add (new CBCGPRibbonButton (ID_INSERT_PAGEBREAK, _T("Page Break"), 2, 2));

	CBCGPRibbonPanel* pPanelInsertTable = pCategory->AddPanel (
		_T("Tables"), m_PanelImages.ExtractIcon (7));

	pPanelInsertTable->SetKeys (_T("zb"));
	...
	CBCGPRibbonPanel* pPanelIllustrations = pCategory->AddPanel (
		_T("Illustrations"), m_PanelImages.ExtractIcon (8));

	pPanelIllustrations->SetKeys (_T("zi"));

	CBCGPRibbonButton* pInsertPictureBtn = new CBCGPRibbonButton (
		ID_INSERT_PICTURE, _T("Picture"), 4, 4);
	pPanelIllustrations->Add (pInsertPictureBtn);

	pPanelIllustrations->Add (new CBCGPRibbonButton (ID_INSERT_CLIPART, _T("Clip Art"), 5, 5));

	CBCGPRibbonPaletteButton* pInsertShapesBtn = 
		new CBCGPRibbonPaletteButton (ID_INSERT_SHAPES, _T("Shapes"), 6, 6);
	...
	pInsertShapesBtn->AddSubItem (new CBCGPRibbonButton (ID_INSERT_SHAPESNEW, _T("&New Drawing Canvas"), 29, -1));

	pPanelIllustrations->Add (pInsertShapesBtn);

	pPanelIllustrations->Add (new CBCGPRibbonButton (ID_INSERT_SMARTART, _T("SmartArt"), 7, 7));
	pPanelIllustrations->Add (new CBCGPRibbonButton (ID_INSERT_CHART, _T("Chart"), 8, 8));
	
	CBCGPRibbonPanel* pPanelLinks = pCategory->AddPanel (
		_T("Links"), m_PanelImages.ExtractIcon (9));

	pPanelLinks->SetKeys (_T("zl"));

	pPanelLinks->Add (new CBCGPRibbonButton (ID_INSERT_HYPERLINK, _T("Hyperlink"), 9, 9));
	pPanelLinks->Add (new CBCGPRibbonButton (ID_INSERT_BOOKMARK, _T("Bookmark"), 10, 10));
	pPanelLinks->Add (new CBCGPRibbonButton (ID_INSERT_REFERENCE, _T("Cross-reference"), 11, 11));

	CBCGPRibbonPanel* pPanelHeaderFooter = pCategory->AddPanel (
		_T("Header & Footer"), m_PanelImages.ExtractIcon (10));

	pPanelHeaderFooter->SetKeys (_T("zh"));

	CBCGPRibbonButton* pInsertHeaderBtn = new CBCGPRibbonButton (ID_INSERT_HEADER, _T("Header"), 12, 12);
	pInsertHeaderBtn->SetMenu (IDR_INSERT_HEADER_MENU);
	pPanelHeaderFooter->Add (pInsertHeaderBtn);

	CBCGPRibbonButton* pInsertFooterBtn = new CBCGPRibbonButton (ID_INSERT_FOOTER, _T("Footer"), 13, 13);
	pInsertFooterBtn->SetMenu (IDR_INSERT_FOOTER_MENU);
	pPanelHeaderFooter->Add (pInsertFooterBtn);

	CBCGPRibbonButton* pInsertPageNumbersBtn = new CBCGPRibbonButton (ID_INSERT_PAGENUMBER, _T("Page Number"), 14, 14);
	...
	CBCGPRibbonPanel* pPanelText = pCategory->AddPanel (
		_T("Text"), m_PanelImages.ExtractIcon (11));

	pPanelText->SetKeys (_T("zt"));

	CBCGPRibbonButton* pTextBoxBtn = new CBCGPRibbonButton (ID_INSERT_TEXTBOX, _T("Text Box"), 15, 15);
	pTextBoxBtn->SetMenu (IDR_INSERT_TEXTBOX_MENU);
	pPanelText->Add (pTextBoxBtn);

	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_QUICKPARTS, _T("Quick Parts"), 16, 16));
	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_WORDART, _T("WordArt"), 17, 17));
	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_DROPCAP, _T("Drop Cap"), 18, 18));
	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_SIGNATURELINE, _T("Signature Line"), 19));
	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_DATETIME, _T("Date && Time"), 20));
	pPanelText->Add (new CBCGPRibbonButton (ID_INSERT_OBJECT, _T("Object"), 21));
	...
	CBCGPRibbonPanel* pPanelSymbols = pCategory->AddPanel (
		_T("Symbols"), m_PanelImages.ExtractIcon (12));

	pPanelSymbols->SetKeys (_T("zs"));

	pPanelSymbols->Add (new CBCGPRibbonButton (ID_INSERT_EQUATION, _T("Equation"), 22, 19));
	pPanelSymbols->Add (new CBCGPRibbonButton (ID_INSERT_SYMBOL, _T("Symbol"), 23, 20));
...
//	pPanelSymbols->SetElementMenu (ID_INSERT_EQUATION, IDR_DUMMY_MENU, TRUE);
	...
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_REMOVECURRENTCOVERPAGE, _T("Remove Current Cover Page"), 24));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_SAVETOCOVERPAGEGALLERY, _T("Save Selection to Cover Page Gallery"), 25));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_TABLE, _T("Insert Table"), 3));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_DRAWTABLE, _T("Draw Table"), 26));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_CONVERTTOTABLE, _T("Convert to Table"), 27));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_EXCELSPREADSHEET, _T("Excel Spreadsheet"), 28));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_TABLE_QUICKTABLES, _T("Quick Tables"), 3));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_EDITHEADER, _T("Edit Header"), 12));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_REMOVEHEADER, _T("Remove Header"), 30));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_SAVESELECTIONTOHEADERGALLERY, _T("Save Selection to Header Gallery"), 31));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_EDITFOOTER, _T("Edit Footer"), 13));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_REMOVEFOOTER, _T("Remove Footer"), 32));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_SAVESELECTIONTOFOOTERGALLERY, _T("Save Selection to Footer Gallery"), 33));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_TOPOFPAGE, _T("Top of Page"), 34));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_BOTTOMOFPAGE, _T("Bottom of Page"), 35));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_PAGEMARGINS, _T("Page Margins"), 36));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_CURRENTPOSITION, _T("Current Position"), 14));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_FORMATPAGENUMBERS, _T("Format Page Numbers"), 37));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_REMOVEPAGENUMBERS, _T("Remove Page Numbers"), 38));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_DRAWTEXTBOX, _T("Draw Text Box"), 15));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_INSERT_SAVESELECTIONTOTEXTBOXGALLERY, _T("Save Selection to Text Box Gallery"), 39));
...
void CMainFrame::AddTab_PageLayout ()
{
	...
	CBCGPRibbonPanel* pPanelThemes = pCategory->AddPanel (
	...
	CBCGPRibbonPanel* pPanelPageSetup = pCategory->AddPanel (
		_T("Page Setup\nzs"), m_PanelImages.ExtractIcon (14));

	CBCGPRibbonButton* pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_MARGINS, _T("Margins\nm"), 3, 1);
	pBtn->SetMenu (IDR_PAGELAYOUT_MARGINS);
	pPanelPageSetup->Add (pBtn);

	pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_ORIENTATION, _T("Orientation\no"), 4, 2);
	pBtn->SetMenu (IDR_PAGELAYOUT_ORIENTATION);
	pPanelPageSetup->Add (pBtn);

	pPanelPageSetup->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGESETUP_SIZE, _T("Size\nsz"), 5, 3));
	pPanelPageSetup->SetElementMenu (ID_PAGELAYOUT_PAGESETUP_SIZE, IDR_DUMMY_MENU);

	pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_COLUMNS, _T("Columns\nj"), 6, 4);
	pBtn->SetMenu (IDR_PAGELAYOUT_COLUMNS);
	pPanelPageSetup->Add (pBtn);

	pPanelPageSetup->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGESETUP_BREAKS, _T("Breaks\nb"), 7, -1));
	pPanelPageSetup->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGESETUP_LINENUMBERS, _T("Line Numbers\nln"), 8, -1));
	pPanelPageSetup->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGESETUP_HYPHENATION, _T("Hyphenation\nh"), 9, -1));
	...
	pPanelPageSetup->EnableLaunchButton (ID_PARA_PAGESETUP, -1, _T("sp"));

	CBCGPRibbonPanel* pPanelPageBackground = pCategory->AddPanel (
		_T("Page Background\nzb"), m_PanelImages.ExtractIcon (15));

	pPanelPageBackground->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGEBACKGROUND_WATERMARK, _T("Watermark\npw"), 10, 5));
	...
	pPanelPageBackground->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_PAGEBACKGROUND_PAGEBORDERS, _T("Page Borders\npb"), 12, 7));
	pPanelPageBackground->SetElementMenu (ID_PAGELAYOUT_PAGEBACKGROUND_WATERMARK, IDR_DUMMY_MENU);

	CBCGPRibbonPanel* pPanelParagraph = pCategory->AddPanel (
		_T("Paragraph\nzg"), m_PanelImages.ExtractIcon (16));

	pPanelParagraph->Add (new CBCGPRibbonLabel (_T("Indent")));

	CBCGPRibbonEdit* pEditIndentLeft = new CBCGPRibbonEdit (ID_PAGELAYOUT_INDENT_LEFT, 72, _T("Left:\nil"), 13);
	pEditIndentLeft->EnableSpinButtons (0, 1000);
	pEditIndentLeft->SetEditText (_T("0"));
	pPanelParagraph->Add (pEditIndentLeft);

	CBCGPRibbonEdit* pEditIndentRight = new CBCGPRibbonEdit (ID_PAGELAYOUT_INDENT_RIGHT, 72, _T("Right:\nir"), 14);
	...
	pPanelParagraph->Add (new CBCGPRibbonLabel (_T("Spacing")));

	CBCGPRibbonEdit* pEditSpaceBefore = new CBCGPRibbonEdit (ID_PAGELAYOUT_SPACE_BEFORE, 72, _T("Before:\nsb"), 15);
	pEditSpaceBefore->EnableSpinButtons (0, 100);
	pEditSpaceBefore->SetEditText (_T("0"));
	pPanelParagraph->Add (pEditSpaceBefore);

	CBCGPRibbonEdit* pEditSpaceAfter = new CBCGPRibbonEdit (ID_PAGELAYOUT_SPACE_AFTER, 72, _T("After:\nsa"), 16);
	...
	CBCGPRibbonPanel* pPanelArrange = AddPanelArrange (pCategory, 17, 17, 8);

	pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_COLUMNS_MORECOLUMNS, _T("More Columns"), 6));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_BRINGFORWARD, _T("Bring Forward"), 26));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_BRINGINFRONTOFTEXT, _T("Bring in Front of Text"), 27));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_SENDBACKWARD, _T("Send Backward"), 28));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_SENDBEHINDTEXT, _T("Send Behind Text"), 29));

    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_INLINEWITHTEXT, _T("In Line With Text"), 30));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_SQUARE, _T("Square"), 31));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_TIGHT, _T("Tight"), 32));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_BEHINDTEXT, _T("Behind Text"), 33));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_INFRONTOFTEXT, _T("In Front of Text"), 34));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_TOPANDBOTTOM, _T("Top and Bottom"), 35));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_THROUGH, _T("Through"), 36));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING_EDITWRAPPOINTS, _T("Edit Wrap Points"), 37));
    pCategory->AddHidden (new CBCGPRibbonButton (ID_PAGELAYOUT_MORELAYOUTOPTIONS, _T("More Layout Options"), 38));
...
void CMainFrame::AddTab_References ()
{
	...
	CBCGPRibbonButton* pBtn = new CBCGPRibbonButton (ID_TABLE_OF_CONTEXT, _T("Table of Contents\nt"), 12, 0);
	pBtn->SetMenu (IDR_DUMMY_MENU);
	pPanelRef_TC->Add (pBtn);

	pBtn = new CBCGPRibbonButton (ID_REFERENCES_ADDTEXT, _T("Add Text\na"), 0, -1);
	pBtn->SetMenu (IDR_REFERENCES_ADDTEXT);
	pPanelRef_TC->Add (pBtn);

	pPanelRef_TC->Add (new CBCGPRibbonButton (ID_REFERENCES_TC_UPDATETABLE, _T("Update Table\nu"), 1, -1));

	CBCGPRibbonPanel* pPanelRef_Footnotes = pCategory->AddPanel (
		_T("Footnotes\nzf"), m_PanelImages.ExtractIcon (19));

	pPanelRef_Footnotes->Add (new CBCGPRibbonButton (ID_REFERENCES_FOOTNOTES_INSERTFOOTNOTE, _T("Insert Footnote\nf"), 13, 1));
	pPanelRef_Footnotes->Add (new CBCGPRibbonButton (ID_REFERENCES_FOOTNOTES_INSERTENDNOTE, _T("Insert Endnote\ne"), 2, -1));

	pBtn = new CBCGPRibbonButton (ID_REFERENCES_FOOTNOTE_NEXT, _T("Next Footnote\no"), 3, -1);
	pBtn->SetMenu (IDR_REFERENCES_NEXTFOOTNOTE, TRUE);
	pPanelRef_Footnotes->Add (pBtn);

	pPanelRef_Footnotes->Add (new CBCGPRibbonButton (ID_REFERENCES_FOOTNOTES_SHOWNOTES, _T("Show Notes\nh"), 4, -1));

	CBCGPRibbonPanel* pPanelRef_CB = pCategory->AddPanel (
		_T("Citations & Bibliography\nzb"), m_PanelImages.ExtractIcon (20));

	pBtn = new CBCGPRibbonButton (ID_INSERT_CITATION, _T("Insert Citation\nc"), 14, 2);
	pBtn->SetMenu (IDR_REFERENCES_INSERTCITATION);
	pPanelRef_CB->Add (pBtn);

	pPanelRef_CB->Add (new CBCGPRibbonButton (ID_REFERENCES_CB_MANAGESOURCES, _T("Manage Sources\nm"), 5, -1));

	// Add "Style" combo:
	CBCGPRibbonComboBox* pStyleCombo = new CBCGPRibbonComboBox (ID_REFERENCES_STYLE, TRUE, 45,
		_T("Style: \nl"), 6);
	...
	pPanelRef_CB->Add (new CBCGPRibbonButton (ID_REFERENCES_CB_INSERTBIBLIOGRAPHY, _T("Bibliography\nb"), 7, -1));
	...
	pPanelRef_Captions->Add (new CBCGPRibbonButton (ID_REFERENCES_CAPTIONS_INSERTCAPTION, _T("Insert Caption\np"), 15, 3));
	pPanelRef_Captions->Add (new CBCGPRibbonButton (ID_REFERENCES_CAPTIONS_INSERTTOF, _T("Insert Table of Figures\ng"), 8, -1));
	pPanelRef_Captions->Add (new CBCGPRibbonButton (ID_REFERENCES_CAPTIONS_UPDATETABLE, _T("Update Table\nv"), 1, -1));
	pPanelRef_Captions->Add (new CBCGPRibbonButton (ID_REFERENCES_CAPTIONS_CROSSREFERENCE, _T("Cross-reference\nrf"), 9, -1));

	CBCGPRibbonPanel* pPanelRef_Index = pCategory->AddPanel (
		_T("Index\nzi"), m_PanelImages.ExtractIcon (22));

	pPanelRef_Index->Add (new CBCGPRibbonButton (ID_REFERENCES_INDEX_MARKENTRY, _T("Mark Entry\nn"), 16, 4));
	pPanelRef_Index->Add (new CBCGPRibbonButton (ID_REFERENCES_INDEX_INSERTINDEX, _T("Insert Index\nx"), 10, -1));
	pPanelRef_Index->Add (new CBCGPRibbonButton (ID_REFERENCES_INDEX_UPDATEINDEX, _T("Update Index\nd"), 1, -1));

	CBCGPRibbonPanel* pPanelRef_TA = pCategory->AddPanel (
		_T("Table of Authorities\nza"), m_PanelImages.ExtractIcon (23));

	pPanelRef_TA->Add (new CBCGPRibbonButton (ID_REFERENCES_TA_MARKCITATION, _T("Mark Citation\ni"), 17, 5));
	pPanelRef_TA->Add (new CBCGPRibbonButton (ID_REFERENCES_TA_INSERTTOA, _T("Insert Table of Authorities\nrt"), 11, -1));
	pPanelRef_TA->Add (new CBCGPRibbonButton (ID_REFERENCES_TA_UPDATETABLE, _T("Update Table\nru"), 1, -1));
}
//***********************************************************************************
void CMainFrame::AddTab_Mailings ()
{
	...
	pPanelCreate->Add (new CBCGPRibbonButton (ID_MAILINGS_CREATE, _T("Envelopes\ne"), 0, 0));
	pPanelCreate->Add (new CBCGPRibbonButton (ID_MAILINGS_CREATELABELS, _T("Labels\nl"), 1, 1));

	CBCGPRibbonPanel* pPanelStartMerge = pCategory->AddPanel (
		_T("Start Mail Merge\nzs"), m_PanelImages.ExtractIcon (25));

	CBCGPRibbonButton* pMailMergeMenuBtn = new CBCGPRibbonButton (ID_MAILINGS_STARTMAILMERGE, _T("Start Mail Merge\ns"), 2, 2);
	pMailMergeMenuBtn->SetMenu (IDR_MAIL_MERGE_MENU);
	pPanelStartMerge->Add (pMailMergeMenuBtn);

	CBCGPRibbonButton* pSelectRecipientMenuBtn = new CBCGPRibbonButton (ID_MAILINGS_SELECTRECIPIENTS, _T("Select Recipients\nr"), 3, 3);
	pSelectRecipientMenuBtn->SetMenu (IDR_MAIL_RECIPIENTS_MENU);
	pPanelStartMerge->Add (pSelectRecipientMenuBtn);

	pPanelStartMerge->Add (new CBCGPRibbonButton (ID_MAILINGS_EDITRECIPIENTLIST, _T("Edit Recipient List\nd"), 4, 4));

	CBCGPRibbonPanel* pPanelWriteFields = pCategory->AddPanel (
		_T("Write & Insert Fields\nzw"), m_PanelImages.ExtractIcon (26));

	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_HIGHLIGHT, _T("Highlight Merge Fields\nh"), 5, 5));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_ADDRESSBLOCK, _T("Address Block\na"), 6, 6));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_GREATINGLINE, _T("Greating Line\ng"), 7, 7));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_INSERTMERGEDFIELD, _T("Insert Merge Field\ni"), 8, 8));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_RULES, _T("Rules\nu"), 9));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_MATCHFIELDS, _T("Match Fields\nt"), 10));
	pPanelWriteFields->Add (new CBCGPRibbonButton (ID_MAILINGS_UPDATELABELS, _T("Update Labels\nb"), 11));
	...
	pPanelReview->Add (new CBCGPRibbonButton (ID_MAILINGS_VIEWMERGEDDATA, _T("Preview Results\np"), 12, 9));
	pPanelReview->AddSeparator ();

	CBCGPRibbonButtonsGroup* pButtonsListRecords = new CBCGPRibbonButtonsGroup;

	pButtonsListRecords->AddButton (new CBCGPRibbonButton (ID_MAILINGS_FIRSTRECORD, _T("\nq"), 16));
	pButtonsListRecords->AddButton (new CBCGPRibbonButton (ID_MAILINGS_PREVIOUSRECORD, _T("\nm"), 17));

	CBCGPRibbonEdit* pEdit = new CBCGPRibbonEdit (ID_MAILINGS_GOTORECORD, 40);
	pEdit->SetKeys (_T("w"));
	pButtonsListRecords->AddButton (pEdit);

	pButtonsListRecords->AddButton (new CBCGPRibbonButton (ID_MAILINGS_NEXTRECORD, _T("\nx"), 18));
	pButtonsListRecords->AddButton (new CBCGPRibbonButton (ID_MAILINGS_LASTRECORD, _T("\nv"), 19));

	pPanelReview->Add (pButtonsListRecords);

	pPanelReview->Add (new CBCGPRibbonButton (ID_MAILINGS_FINDRECIPIENT, _T("Find Recipient\nj"), 13));
	pPanelReview->Add (new CBCGPRibbonButton (ID_MAILINGS_AUTOCHECHERRORS, _T("Auto Check for Errors\nk"), 14));

	CBCGPRibbonPanel* pPanelFinish = pCategory->AddPanel (_T("Finish\nzf"), m_PanelImages.ExtractIcon (28));
	pPanelFinish->Add (new CBCGPRibbonButton (ID_MAILINGS_FINISH, _T("Finish && Merge\nf"), 15, 10));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_TYPENEWLIST, _T("Type New List..."), 27));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_USEEXISTINGLIST, _T("Use Existing List..."), 28));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_SELECTFROMCONTACTS, _T("Select from Contacts..."), 29));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_LETTERS, _T("Letters"), 20));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_EMAILMESSAGES, _T("E-mail Messages"), 21));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_ENVELOPES, _T("Envelops..."), 22));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_LABELS, _T("Labels..."), 23));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_DIRECTORY, _T("Directory"), 24));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_NORMALDOCUMENT, _T("Normal Document"), 25));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_MAILINGS_STEPBYSTEPMAILMERGEWIZARD, _T("Step by Step Mail Merge Wizard..."), 26));
}
//***********************************************************************************
void CMainFrame::AddTab_Review ()
{
	...
	CBCGPRibbonPanel* pPanelProofing = pCategory->AddPanel (
		_T("Proofing"), m_PanelImages.ExtractIcon (29));

	pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_SPEELINGGRAMMAR, _T("Spelling && Grammar\ns"), 0, 0));
	pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_RESEARCH, _T("Research\nr"), 1, 1));
    pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_THESAURUS, _T("Thesaurus\ne"), 2, 2));
    pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_TRANSLATE, _T("Translate\nl"), 3, 3));
    pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_TRANSLATIONSCREENTIP, _T("Translation ScreenTip\npt"), 4, -1));
    pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_SETLANGUAGE, _T("Set Language\nu"), 5, -1));
    pPanelProofing->Add (new CBCGPRibbonButton (ID_REVIEW_PROOFING_WORDCOUNT, _T("Word Count\nw"), 6, -1));

	pPanelProofing->SetElementMenu (ID_REVIEW_PROOFING_TRANSLATIONSCREENTIP, IDR_DUMMY_MENU);

	CBCGPRibbonPanel* pPanelComments = pCategory->AddPanel (
		_T("Comments"), m_PanelImages.ExtractIcon (30));

	pPanelComments->Add (new CBCGPRibbonButton (ID_REVIEW_COMMENTS_NEWCOMMENT, _T("New Comment\nc"), 7, 4));
	
	CBCGPRibbonButton* pDeleteBtn = new CBCGPRibbonButton (ID_REVIEW_COMMENTS_DELETE, _T("Delete\nd"), 8, 5);
	pDeleteBtn->SetMenu (IDR_REVIEW_COMMENTS_DELETE, TRUE);
	pPanelComments->Add (pDeleteBtn);

	pPanelComments->Add (new CBCGPRibbonButton (ID_REVIEW_COMMENTS_PREVIOUS, _T("Previous\nv"), 9, 6));
	pPanelComments->Add (new CBCGPRibbonButton (ID_REVIEW_COMMENTS_NEXT, _T("Next\nn"), 10, 7));

	CBCGPRibbonPanel* pPanelTracking = pCategory->AddPanel (
		_T("Tracking"), m_PanelImages.ExtractIcon (31));

	CBCGPRibbonButton* pTCButton = new CBCGPRibbonButton (ID_REVIEW_TRACKING_TRACKCHANGES, _T("Track Changes\ng"), 11, 8);
	pTCButton->SetMenu (IDR_REVIEW_TRACKING_TRACKCHANGES, TRUE);
	pTCButton->SetAlwaysLargeImage ();
	pPanelTracking->Add (pTCButton);

	CBCGPRibbonButton* pBalloonsBtn = new CBCGPRibbonButton (ID_REVIEW_TRACKING_BALLOONS, _T("Balloons\ntb"), 12, 9);
	pBalloonsBtn->SetMenu (IDR_REVIEW_TRACKING_BALLOONS);
	pBalloonsBtn->SetAlwaysLargeImage ();
	pPanelTracking->Add (pBalloonsBtn);

	CBCGPRibbonComboBox* pDisplayCombo = new CBCGPRibbonComboBox (ID_REVIEW_TRACKING_DISPLAYFORREVIEW, FALSE, 149, NULL, 13);
	...
	CBCGPRibbonButton* pShowBtn = new CBCGPRibbonButton (ID_REVIEW_TRACKING_SHOW_MARKUP, _T("Show Markup\ntm"), 14, -1);
	pShowBtn->SetMenu (IDR_REVIEW_TRACKING_SHOW);
	pPanelTracking->Add (pShowBtn);

	CBCGPRibbonButton* pPaneBtn = new CBCGPRibbonButton (ID_REVIEW_TRACKING_REVIEWINGPANE, _T("Reviewing Pane\ntp"), 15, -1);
	pPaneBtn->SetMenu (IDR_REVIEW_TRACKING_REVIEWINGPANE, TRUE);
	pPanelTracking->Add (pPaneBtn);

	CBCGPRibbonPanel* pPanelChanges = pCategory->AddPanel (
		_T("Changes"), m_PanelImages.ExtractIcon (32));

	pPanelChanges->SetCenterColumnVert ();

	CBCGPRibbonButton* pAcceptBtn = new CBCGPRibbonButton (ID_REVIEW_CHANGES_ACCEPT, _T("Accept\na"), 16, 10);
	pAcceptBtn->SetMenu (IDR_REVIEW_CHANGES_ACCEPT, TRUE);
	pPanelChanges->Add (pAcceptBtn);

	CBCGPRibbonButton* pRejectBtn = new CBCGPRibbonButton (ID_REVIEW_CHANGES_REJECT, _T("Reject\nj"), 17, 11);
	pRejectBtn->SetMenu (IDR_REVIEW_CHANGES_REJECT, TRUE);
	pPanelChanges->Add (pRejectBtn);

	pPanelChanges->Add (new CBCGPRibbonButton (ID_REVIEW_CHANGES_PREVIOUS, _T("Previous\nf"), 18, -1));
	pPanelChanges->Add (new CBCGPRibbonButton (ID_REVIEW_CHANGES_NEXT, _T("Next\nh"), 19, -1));

	CBCGPRibbonPanel* pPanelCompare = pCategory->AddPanel (
		_T("Compare"), m_PanelImages.ExtractIcon (33));

	CBCGPRibbonButton* pCompareBtn = new CBCGPRibbonButton (ID_REVIEW_COMPARE_COMPARE, _T("Compare\nm"), 20, 12);
	pCompareBtn->SetMenu (IDR_REVIEW_COMPARE_COMPARE_COMPARE);
	pPanelCompare->Add (pCompareBtn);

	CBCGPRibbonButton* pShowSrcBtn = new CBCGPRibbonButton (ID_REVIEW_COMPARE_SHOWSOURCE, _T("Show Source Documents\no"), 21, 13);
	pShowSrcBtn->SetMenu (IDR_REVIEW_COMPARE_SHOWSOURCE);
	pPanelCompare->Add (pShowSrcBtn);

	CBCGPRibbonPanel* pPanelProtect = pCategory->AddPanel (
		_T("Protect"), m_PanelImages.ExtractIcon (34));

	pPanelProtect->Add (new CBCGPRibbonButton (ID_REVIEW_PROTECT, _T("Protect Document\npr"), 22, 14));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_TRACKING_REVIEWINGPANE_VERTICAL, _T("Reviewing Pane Vertical"), 15));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_TRACKING_REVIEWINGPANE_HORIZONTAL, _T("Reviewing Pane Horizontal"), 23));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_CHANGES_ACCEPT_CHANGE, _T("Accept Change"), 16));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_CHANGES_REJECT_CHANGE, _T("Reject Change"), 17));

	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_COMPARE_COMPARE_COMPARE, _T("Compare"), 24));
	pCategory->AddHidden (new CBCGPRibbonButton (ID_REVIEW_COMPARE_COMPARE_COMBINE, _T("Combine"), 25));
}
//***********************************************************************************
void CMainFrame::AddTab_View ()
{
	...
	pPanelDocViews->Add (new CBCGPRibbonButton (ID_VIEW_PRINTLAYOUT, _T("Print Layout\np"), 7, 0));
	pPanelDocViews->Add (new CBCGPRibbonButton (ID_VIEW_FULLSCREEN, _T("Full Screen Reading\nf"), 4, 1));
	pPanelDocViews->Add (new CBCGPRibbonButton (ID_VIEW_WEBLAYOUT, _T("Web Layout\nl"), 12, 2));
	pPanelDocViews->Add (new CBCGPRibbonButton (ID_VIEW_OUTLINE, _T("Outline\nu"), 6, 3));
	pPanelDocViews->Add (new CBCGPRibbonButton (ID_VIEW_DRAFT, _T("Draft\ne"), 3, 4));
	...
	pPanelShowHide->Add (new CBCGPRibbonCheckBox (ID_VIEW_MESSAGEBAR, _T("Message Bar\nd")));
	...
	pPanelZoom->Add (new CBCGPRibbonButton (ID_VIEW_ZOOM, _T("Zoom\nq"), 13, 5));
	pPanelZoom->Add (new CBCGPRibbonButton (ID_VIEW_ZOOM_100, _T("100%\nj"), 2, 10));
	pPanelZoom->Add (new CBCGPRibbonButton (ID_VIEW_ZOOM_ONE_PAGE, _T("One Page\n1"), 14));
	pPanelZoom->Add (new CBCGPRibbonButton (ID_VIEW_ZOOM_TWO_PAGES, _T("Two Pages\n2"), 15));
	pPanelZoom->Add (new CBCGPRibbonButton (ID_VIEW_ZOOM_PAGE_WIDTH, _T("Page Width\ni"), 16));

	CBCGPRibbonPanel* pPanelWindow = pCategory->AddPanel (
		_T("Window\nzw"), m_PanelImages.ExtractIcon (38));

	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_NEWWINDOW, _T("New Window\nn"), 5, 6));
	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_ARRANGEALL, _T("Arrange All\na"), 0, 7));
	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_SPLIT, _T("Split\ns"), 9, 8));
	pPanelWindow->AddSeparator ();
	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_COMPARE, _T("View Side by Side\nb"), 11));
	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_SYNCSCROLLING, _T("Synchronous Scrolling\nvs"), 10));
	pPanelWindow->Add (new CBCGPRibbonButton (ID_VIEW_RESETWINDOW, _T("Reset Window Position\nt"), 8));
	pPanelWindow->AddSeparator ();

	CBCGPRibbonButton* pWindowsBtn = new CBCGPRibbonButton (ID_VIEW_SWITCHWINDOW, _T("Switch Windows\nw"), 1, 9);
	pWindowsBtn->SetAlwaysLargeImage ();
	pWindowsBtn->SetMenu (IDR_DUMMY_MENU);
	pPanelWindow->Add (pWindowsBtn);

	CBCGPRibbonPanel* pPanelMacros = pCategory->AddPanel (
		_T("Macros\nzm"), m_PanelImages.ExtractIcon (39));
	pPanelMacros->Add (new CBCGPRibbonButton (ID_VIEW_MACROS, _T("Macros\nm"), -1, 11));
...
void CMainFrame::CreateLookColor ()
{
	m_pLookColor = new CBCGPRibbonColorButton (ID_VIEW_APPLOOK_2007_5, _T("Office 2007 (C&ustom Color)..."), TRUE, -1, -1);
	m_pLookColor->SetToolTipText (_T("Office 2007 Look"));
	m_pLookColor->SetDescription (_T("Activate Office 2007 application look"));
	m_pLookColor->SetColumns (10);
	m_pLookColor->SetColorBoxSize (CSize (17, 17));
	m_pLookColor->EnableAutomaticButton (_T("&Automatic"), theApp.m_clrCustomDef, TRUE, _T("Automatic"));
	m_pLookColor->EnableOtherButton (_T("&More Colors..."), _T("More Colors"));
...
CBCGPRibbonPanel* CMainFrame::AddPanelArrange (CBCGPRibbonCategory* pCategory, UINT imageP, UINT imageS, UINT imageL)
{
	...
	CBCGPRibbonPaletteButton* pPositionBtn = 
		new CBCGPRibbonPaletteButton (ID_PAGELAYOUT_ARRANGE_POSITION, _T("Position\npo"), -1, imageL);
	InitPositionPalette (pPositionBtn);
	pPositionBtn->EnableMenuResize ();
	pPositionBtn->SetButtonMode ();
	pPanelPL_Arrange->Add (pPositionBtn);

	CBCGPRibbonButton* pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_BRINGTOFRONT, _T("Bring to Front\naf"), imageS, imageL > 0 ? imageL + 1 : -1);
	pBtn->SetMenu (IDR_PAGELAYOUT_BRINGTOFRONT, TRUE);
	pPanelPL_Arrange->Add (pBtn);

	pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_SENDTOBACK, _T("Send to Back\nab"), imageS + 2, imageL > 0 ? imageL + 2 : -1);
	pBtn->SetMenu (IDR_PAGELAYOUT_SENDTOBACK, TRUE);
	pPanelPL_Arrange->Add (pBtn);

	pBtn = new CBCGPRibbonButton (ID_PAGELAYOUT_TEXTWRAPPING, _T("Text Wrapping\ntw"), imageS + 4, imageL > 0 ? imageL + 3 : -1);
	pBtn->SetMenu (IDR_PAGELAYOUT_TEXTWRAPPING, FALSE);
	pPanelPL_Arrange->Add (pBtn);

	pPanelPL_Arrange->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_ARRANGE_ALIGN, _T("Align\naa"), imageS + 1, imageL > 0 ? imageL + 4 : -1));
	pPanelPL_Arrange->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_ARRANGE_GROUP, _T("Group\nag"), imageS + 3, imageL > 0 ? imageL + 5 : -1));
	pPanelPL_Arrange->Add (new CBCGPRibbonButton (ID_PAGELAYOUT_ARRANGE_ROTATE, _T("Rotate\nay"), imageS + 5, imageL > 0 ? imageL + 6 : -1));
...
void CMainFrame::AddContextTab_Picture ()
{
	...
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_BRIGHTNESS, _T("Brightness\nb"), 0, -1));
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_CONTRAST, _T("Contrast\nn"), 1, -1));
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_RECOLOR, _T("Recolor\ne"), 2, -1));
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_COMPRESS, _T("Compress Pictures\nm"), 3, -1));
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_CHANGE, _T("Change Picture\ng"), 4, -1));
	pPanelTools->Add (new CBCGPRibbonButton (ID_PICTURE_FORMAT_TOOLS_RESET, _T("Reset Picture\no"), 5, -1));
...
void CMainFrame::AddContextTab_Chart ()
{
	...
	pPanelDesign_Type->Add (new CBCGPRibbonButton (ID_CHART_DESIGN_TYPE_CHANGE, _T("Change Chart Type\nc"), -1, 0));
	pPanelDesign_Type->Add (new CBCGPRibbonButton (ID_CHART_DESIGN_TYPE_SAVETEMPLATE, _T("Save Template\nt"), -1, 1));

	CBCGPRibbonPanel* pPanelDesign_Data = pCategory->AddPanel (
		_T("Data"), m_PanelImages.ExtractIcon (20));

	pPanelDesign_Data->Add (new CBCGPRibbonButton (ID_CHART_DESIGN_DATA_SWITCH, _T("Switch Row/Column\nw"), -1, 2));
	pPanelDesign_Data->Add (new CBCGPRibbonButton (ID_CHART_DESIGN_DATA_EDIT, _T("Edit Data Source\ne"), -1, 3));
	pPanelDesign_Data->Add (new CBCGPRibbonButton (ID_CHART_DESIGN_DATA_SHOW, _T("Show Data\nd"), -1, 4));
	...
	pPanelLayout_Shapes->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_SHAPES_PICTURE, _T("Picture\np"), 0, 0));
	pPanelLayout_Shapes->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_SHAPES_SHAPES, _T("Shapes\nsh"), 1, 1));
	pPanelLayout_Shapes->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_SHAPES_TEXTBOX, _T("Draw Text Box\nx"), 2, 2));

	CBCGPRibbonPanel* pPanelLayout_Labels = pCategory2->AddPanel (
		_T("Labels"), m_PanelImages.ExtractIcon (20));

	pPanelLayout_Labels->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_LABELS_TITLE, _T("Chart Title\nt"), -1, 3));
	pPanelLayout_Labels->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_LABELS_AXIS, _T("Axis Titles\ni"), -1, 4));
	pPanelLayout_Labels->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_LABELS_LEGEND, _T("Legend\nl"), 3, 5));
	pPanelLayout_Labels->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_LABELS_DATALABELS, _T("Data Labels\nb"), 4, 6));
	pPanelLayout_Labels->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_LABELS_DATATABLE, _T("Data Table\nd"), 5, 7));

	CBCGPRibbonPanel* pPanelLayout_Axes = pCategory2->AddPanel (
		_T("Axes"), m_PanelImages.ExtractIcon (20));

	pPanelLayout_Axes->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_AXES_AXES, _T("Axes\na"), -1, 8));
	pPanelLayout_Axes->Add (new CBCGPRibbonButton (ID_CHART_LAYOUT_AXES_GRIDLINES, _T("Gridlines\ng"), -1, 9));
	...
	pPanelFormat_Sel->Add (new CBCGPRibbonButton (ID_CHART_FORMAT_CURSEL_FORMAT, _T("Format Selection\nm"), 0, -1));
	pPanelFormat_Sel->Add (new CBCGPRibbonButton (ID_CHART_FORMAT_CURSEL_RESET, _T("Reset to Match Style\nv"), 1, -1));

	CBCGPRibbonPanel* pPanelFormat_Shape = pCategory3->AddPanel (
		_T("Shape Styles"), m_PanelImages.ExtractIcon (20));

	pPanelFormat_Shape->Add (new CBCGPRibbonButton (ID_CHART_FORMAT_SHAPES_FILL, _T("Shape Fill\nsf"), 2, -1));
	pPanelFormat_Shape->Add (new CBCGPRibbonButton (ID_CHART_FORMAT_SHAPES_OUTLINE, _T("Shape Outline\nso"), 3, -1));
	pPanelFormat_Shape->Add (new CBCGPRibbonButton (ID_CHART_FORMAT_SHAPES_EFFECTS, _T("Shape Effects\nse"), 4, -1));
...
BOOL CMainFrame::CreateStatusBar ()
{
	...
	m_wndStatusBar.AddElement (new CBCGPRibbonHyperlink (
		ID_STATUSBAR_LINK, _T("www.bcgsoft.com"), _T("http://www.bcgsoft.com")),
		_T("Link to website"));
	...
	pSBGroup->AddButton (new CBCGPRibbonButton (ID_STATUSBAR_PRINTLAYOUT, _T(""), 0));
	pSBGroup->AddButton (new CBCGPRibbonButton (ID_STATUSBAR_FULL_SCREEN_READING, _T(""), 1));
	pSBGroup->AddButton (new CBCGPRibbonButton (ID_STATUSBAR_WEBLAYOUT, _T(""), 2));
	pSBGroup->AddButton (new CBCGPRibbonButton (ID_STATUSBAR_MASTER_DOCUMENT_TOOLS, _T(""), 3));
	pSBGroup->AddButton (new CBCGPRibbonButton (ID_STATUSBAR_DRAFT, _T(""), 4));
...
BOOL CMainFrame::CreateMessageBar ()
{
	...
	m_wndMessageBar.SetText (_T("Welcome to BCGControlBar Pro demonstration!"), CBCGPCaptionBar::ALIGN_LEFT);

	m_wndMessageBar.SetBitmap (IDB_INFO, (COLORREF)-1, FALSE, CBCGPCaptionBar::ALIGN_LEFT);
	m_wndMessageBar.SetImageToolTip (_T("Important"), _T("Please take a look at BCGPMSOffice2007Demo source code to learn how to create advanced user interface in minutes."));
...
void CMainFrame::OnViewRtl() 
{
	...
	m_wndClipboardPane.AdjustControls ();
	m_wndClipboardPane.RedrawWindow (NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE | RDW_ALLCHILDREN);
}
...
LRESULT CMainFrame::OnHighlightRibbonListItem (WPARAM wp, LPARAM lp)
{
	...
	case ID_WRITE_QUICKSTYLES:
		strInfo.Format (_T("Modify document style. Index: %d - %s"), nIndex, m_arStyles [nIndex]);
		break;
...
void CMainFrame::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	{
		((CBCGPMSOffice2007DemoView*)GetActiveView ())->ScrollTo (nPos);
	}
}
...
LRESULT CMainFrame::OnCustomizeGalleryContextMenu(WPARAM wp, LPARAM lp)
{
	...
	if (nID == ID_WRITE_QUICKSTYLES)
	{
		...
		{
			CString strItem;
			strItem.Format (_T("Update %s to M&atch Selection"), m_arStyles [nIndex]);
