...
IMPLEMENT_DYNCREATE(CInPlaceFrame, CBCGPOleIPFrameWnd)

BEGIN_MESSAGE_MAP(CInPlaceFrame, CBCGPOleIPFrameWnd)
...
//  ON_COMMAND(ID_CONTEXT_HELP, CBCGOleIPFrameWnd::OnContextHelp)
END_MESSAGE_MAP()
...
{
	if (CBCGPOleIPFrameWnd::OnCreate(lpCreateStruct) == -1)
		return -1;
...
BOOL CInPlaceFrame::OnCreateControlBars(CFrameWnd* pWndFrame, CFrameWnd* /*pWndDoc*/)
{
	...
	CBCGPOleCntrFrameWnd* pBCGFrame = DYNAMIC_DOWNCAST (CBCGPOleCntrFrameWnd, m_pMainFrame);
	...
	ASSERT_VALID (pBCGFrame);

	pBCGFrame->EnableDocking(CBRS_ALIGN_ANY);
	pBCGFrame->DockControlBar(&m_wndToolBar);
	pBCGFrame->DockControlBar(&m_wndFormatBar);
...
{
	CBCGPOleIPFrameWnd::AssertValid();
}

void CInPlaceFrame::Dump(CDumpContext& dc) const
{
	CBCGPOleIPFrameWnd::Dump(dc);
}
#endif //_DEBUG
...
void CInPlaceFrame::RepositionFrame(LPCRECT lpPosRect, LPCRECT lpClipRect)
{
	...
	CBCGPOleIPFrameWnd::RepositionFrame(&rectNew, lpClipRect);
...
void CInPlaceFrame::RecalcLayout(BOOL bNotify)
{
	if (m_wndResizeBar.m_hWnd != NULL)
		m_wndResizeBar.BringWindowToTop();
	CBCGPOleIPFrameWnd::RecalcLayout(bNotify);
	CWnd* pWnd = GetActiveView();
	if (pWnd != NULL)
		pWnd->BringWindowToTop();
...
{
	CBCGPOleIPFrameWnd::CalcWindowRect(lpClientRect, nAdjustType);
}
