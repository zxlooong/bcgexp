...
BEGIN_MESSAGE_MAP(CRulerBar, CBCGPControlBar)
...
void CRulerBar::CreateGDIObjects()
{
	...
	penBtnHighLight.CreatePen(PS_SOLID, 0, globalData.clrBarHilite);
	penBtnShadow.CreatePen(PS_SOLID, 0, globalData.clrBarShadow);
	penWindowFrame.CreatePen(PS_SOLID, 0, GetSysColor(COLOR_WINDOWFRAME));
	penBtnText.CreatePen(PS_SOLID, 0, globalData.clrBarText);
	penBtnFace.CreatePen(PS_SOLID, 0, globalData.clrBarFace);
	...
	brushBtnFace.CreateSolidBrush(globalData.clrBarFace);
}
...
{
	ASSERT(bHorz);
	CSize m_size = CBCGPControlBar::CalcFixedLayout(bStretch, bHorz);
	CRect rectSize;
...
{
	CBCGPControlBar::DoPaint(pDC); // CControlBar::DoPaint -- draws border
	if (m_unit.m_nTPU != 0)
	{
		pDC->SaveDC();
...
{
	CBCGPControlBar::OnWindowPosChanged(lpwndpos);
	m_bDeferInProgress = FALSE;
}

LRESULT CRulerBar::OnSizeParent(WPARAM, LPARAM)
{
	AdjustDockingLayout ();
	return 0;
}
