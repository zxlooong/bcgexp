	...
	CWnd* pParentWnd) : CCSDialog()
{
	...
	if (HIWORD(lpszText) == NULL)
		VERIFY(m_strText.LoadString(LOWORD((DWORD_PTR)lpszText)));
	else
		m_strText = lpszText;
	if (HIWORD(lpszCaption) == NULL)
		VERIFY(m_strCaption.LoadString(LOWORD((DWORD_PTR)lpszCaption)));
	else
...
void CButtonDialog::AddButtons(LPCTSTR lpszButton)
{
	...
	if (HIWORD(lpszButton) == NULL)
		strButtons.LoadString(LOWORD((DWORD_PTR)lpszButton));
	else
