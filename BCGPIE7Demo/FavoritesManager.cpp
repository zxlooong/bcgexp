...
//*******************************************************************************
//
// FavoritesManager.cpp: implementation of the CFavoritesManager class.
//
//////////////////////////////////////////////////////////////////////
...
UINT CFavorit::m_uiNextCommand = ID_FAVORITE_FIRST;
...
BOOL CFavorit::Build (CString strPath)
{
	...
	do
	{
		...
		else if (strName.Right (4).CompareNoCase (_T(".url")) == 0)
		{
			...
			int iIcon = 0;
			if (!m_URLIcons.Lookup (szURL, iIcon))
			{
				// Retrieve icon file
				SHFILEINFO sfi;
				...
				{
					iIcon = sfi.iIcon;
					m_URLIcons.SetAt (szURL, sfi.iIcon);

					if (m_iInternetShortcutIcon == -1)
					{
						int index = ::GetPrivateProfileInt (_T("InternetShortcut"), _T("IconIndex"),
												-32767,
												strFilePath);
		...
		else
		{
			...
			if (pFavorit->CreateMenu (subMenu))
			{
				menu.InsertMenu ((UINT)-1, MF_BYPOSITION | MF_POPUP | MF_STRING, 
					(UINT) subMenu.Detach (), pFavorit->m_strName);
			}
		}
	}
